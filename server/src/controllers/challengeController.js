...
const submitChallenge = async (req, res) => {
    try {
        ...
        // Update weekly stats
        const weekNumber = require('../services/leaderboardService').getCurrentWeekNumber();
        if (!user.weeklyStats || user.weeklyStats.currentWeek !== weekNumber) {
            user.weeklyStats = {
                currentWeek: weekNumber,
                weeklyXP: totalXpEarned,
                weeklyChallenges: 1
            };
        } else {
            user.weeklyStats.weeklyXP += totalXpEarned;
            user.weeklyStats.weeklyChallenges += 1;
        }
        ...
        leaderboardService.updateUserScore(
            ...
            user.xp,
            user.streak,
            user.weeklyStats.weeklyXP
        ).catch(err => console.error('Error updating leaderboard:', err));

        // Check for new achievements (async, don't wait)
        const achievementService = require('../services/achievementService');
        achievementService.checkAchievements(user._id)
            .then(newAchievements => {
                if (newAchievements.length > 0) {
                    console.log(`✅ User ${user._id} unlocked ${newAchievements.length} new achievements`);
                }
            })

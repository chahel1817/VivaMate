  ...
  const addPost = async () => {
    if (newPost.title && newPost.content) {
      try {
        ...
        setShowModal(false);
        setTimeout(() => setPostSubmitted(false), 3000); // Hide after 3 seconds
      } catch (err) {
        ...
        <div className="space-y-6">
          {posts.map((post) => (
            <div key={post._id} className={`${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'} rounded-xl border p-6`}>
              ...
              <p className={`text-sm mb-4 ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>By {post.author} on {new Date(post.date).toLocaleDateString()}</p>
